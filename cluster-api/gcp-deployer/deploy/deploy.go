...
func NewDeployer(provider string, kubeConfigPath string, installationConfigPath string) *deployer {
	...
	ma, err := google.NewMachineActuator(token, nil, installationConfigPath)
	if err != nil {
		glog.Exit(err)
	}
	return &deployer{
